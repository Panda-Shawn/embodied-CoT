...
def split_instance_masks_w_colors(image, mask, instance_id_to_names_and_colors):
    instance_ids = instance_id_to_names_and_colors.keys()
    binary_masks = []
    # import pdb; pdb.set_trace()
    for instance_id in instance_ids:
        if instance_id_to_names_and_colors[instance_id][1] is not None:
            target_rgb = instance_id_to_names_and_colors[instance_id][1]
            tolerance = 80  # 容差范围，可调节，越大越宽松

            distance = np.linalg.norm(image - target_rgb, axis=2)

            binary_mask = (distance < tolerance).astype(np.uint8)
        else:
            binary_mask = (mask == instance_id).astype(np.uint8)
            if instance_id_to_names_and_colors[instance_id][0] == "ball":
                # breakpoint()
                ...
                # 保存图像
                plt.savefig("bef_mask.png", bbox_inches='tight', pad_inches=0)
                plt.close()
                binary_mask[:50, :50] = 0
                plt.figure(figsize=(4, 4))
                plt.imshow(binary_mask, cmap='gray')
                ...
                # 保存图像
                plt.savefig("aft_mask.png", bbox_inches='tight', pad_inches=0)
                plt.close()
                exit()
...
def mask_or_color_to_bboxes(image, mask, instance_id_to_names_and_colors):
    binary_masks, instance_ids = split_instance_masks_w_colors(image, mask, instance_id_to_names_and_colors)
    bboxes = []
    for binary_mask, instance_id in zip(binary_masks, instance_ids):
        ...
        if contours:
            ...
            box = (x, y, x + w, y + h)
            bboxes.append((instance_id_to_names_and_colors[instance_id][0], box))
    return bboxes
