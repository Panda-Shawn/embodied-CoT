...
def main(args):
    ...
    metainfo_json_dict = {}
    metainfo_json_out_path = args.metadata_json_output_path
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        env, task_description = get_libero_env(
            task, "llava", resolution=args.resolution
        )
        ...
        for i in range(len(orig_data.keys())):
            ...
            for _, action in enumerate(orig_actions):
                ...
                agentview_segmentation_instances.append(
            ...
            if done:
                ...
                obs_grp.create_dataset(
                    ...
                    data=np.stack(agentview_segmentation_instances, axis=0),
                )
                ep_data_grp.create_dataset(
                    "task_description", data=task_description.encode("utf-8")
                )
                ep_data_grp.create_dataset(
                    "episode_id",
                    data=f"{args.libero_task_suite}_Task_{task_id}_Demo_{i}".encode(
                        "utf-8"
                    ),
                )
                ep_data_grp.create_dataset(
                    ...
                    data=np.array(
                        [
                            key.encode("utf-8")
                            for key in env.env.model.instances_to_ids.keys()
                        ]
                    ),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help="Path to output JSON file to record success/false and initial states per episode.",
    )
    parser.add_argument(
        "--resolution",
        type=int,
        help="Resolution of images in regenerated dataset. Should be 224 for prism-dinosiglip-224px+7b",
    )
