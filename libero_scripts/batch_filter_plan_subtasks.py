...
if __name__ == "__main__":
    ...
    for episode_id in keys:
        ...
        filtered_plan_subtasks = []
        prev_primitive = None
        filtered_step_mapping = {}
        for step, reasoning in plan_subtasks.items():
            task = reasoning["task"]
            plan = reasoning["plan"]
            subtask = reasoning["subtask"]
            subtask_reason = reasoning["subtask_reason"]
            primitive = reasoning["move"]
            move_reason = reasoning["move_reason"]
            if primitive != prev_primitive:
                filtered_plan_subtasks.append((task, plan, subtask, subtask_reason, primitive, move_reason))
                prev_primitive = primitive
            filtered_step_mapping[step] = len(filtered_plan_subtasks) - 1
        ...
        reasoning_prompt = "trajectory_reasoning = {\n"
        for step, (task, plan, subtask, subtask_reason, primitive, move_reason) in enumerate(filtered_plan_subtasks):
            reasoning_prompt += f'    {step}: "'
            reasoning_prompt += f'<task>{task}</task>'
            reasoning_prompt += f'<plan>{plan}</plan>'
            reasoning_prompt += f'<subtask>{subtask}</subtask>'
            reasoning_prompt += f'<subtask_reason>{subtask_reason}</subtask_reason>'
            reasoning_prompt += f'<move>{primitive}</move>'
            reasoning_prompt += f'<move_reason>{move_reason}</move_reason>'
            reasoning_prompt += '",\n'
        reasoning_prompt += "}"
