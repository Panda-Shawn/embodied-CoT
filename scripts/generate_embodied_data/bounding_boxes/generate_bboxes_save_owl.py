...
parser.add_argument("--output-image-path", default="./annotated_images_owl")  # 新增：保存标注图片的路径
...
def draw_boxes(image, bboxes, output_path):
    ...
    for lg, phrase, box in bboxes:
        ...
        # 绘制边界框
        draw.rectangle([box[1], box[0], box[3], box[2]], outline="red", width=2)
        # 在边界框上方绘制标签和置信度
        label = f"{phrase} ({lg:.2f})"
        draw.text((box[1], box[0] - 10), label, fill="red", font=font)
