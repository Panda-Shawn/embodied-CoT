...
def get_gripper_pos_raw(mask):
    ...
    return (int(pos[0]), int(pos[1])), mask


def get_gripper_mask(images, box):
    if box is None:
        return [None] * len(images)
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        state = predictor.init_state_w_images(images)
    ...
    return episode_masks


def get_gripper_pos_episode(images, gripper_box):
    box = [gripper_box["box"]["xmin"], gripper_box["box"]["ymin"], gripper_box["box"]["xmax"], gripper_box["box"]["ymax"]] if gripper_box is not None else None
    episode_masks = get_gripper_mask(images, box)
    episode_gripper_pos = []
    for mask in episode_masks:
        gripper_pos, _ = get_gripper_pos_raw(mask)
        episode_gripper_pos.append((gripper_pos, mask, gripper_box))

    return episode_gripper_pos


def process_trajectory(episode, gripper_box):
    ...
    gripper_pos_episode = get_gripper_pos_episode(images, gripper_box)

    raw_trajectory = [(*p_m, state) for p_m, state in zip(gripper_pos_episode, states)]
...
def get_corrected_positions_episode(episode, gripper_box, plot=False):

    t = process_trajectory(episode, gripper_box)
...
if __name__=="__main__":
    ...
    gripper_bboxes_json_path = "./gripper_positions/gripper_bboxes.json"

    with open(gripper_bboxes_json_path, "r") as f:
        gripper_bboxes = json.load(f)
    ...
    for ep_idx, episode in enumerate(ds):
        ...
        gripper_bbox = gripper_bboxes[file_path][str(episode_id)]
        print(f"starting ep: {episode_id}, {file_path}")

        pr_pos = get_corrected_positions_episode(episode, gripper_bbox)
